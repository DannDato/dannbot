...
async def end_mail():
    ...
    try:
        ...
        base_users = safe_int(total_users_2)
        pViwers = (incremento_users / base_users) * 100 if base_users > 0 else 0
        ...
        with db_cursor(DB_PATH) as (_, cursor):
            ...
            has_current_chat_table = _table_exists(cursor, table_name)
            has_previous_chat_table = _table_exists(cursor, ptable_name)

            if not has_current_chat_table:
                printlog(f"Tabla de chat no encontrada: {table_name}", "WARNING")
            if not has_previous_chat_table:
                printlog(f"Tabla de chat no encontrada: {ptable_name}", "WARNING")

            # OBTENER EL CONTEO DE LAS PERSONAS QUE CHATEARON EN DIRECTO AL MENOS UNA VEZ
            chat_sources = []
            if has_previous_chat_table:
                chat_sources.append(
                    f"SELECT DISTINCT user FROM {ptable_name} "
                    f"WHERE timestamp BETWEEN DATETIME('{start_time_1}') AND DATETIME('{end_time_1}')"
                )
            if has_current_chat_table:
                chat_sources.append(
                    f"SELECT DISTINCT user FROM {table_name} "
                    f"WHERE timestamp BETWEEN DATETIME('{start_time_1}') AND DATETIME('{end_time_1}')"
                )
            ...
            if chat_sources:
                ...
                nChatters = str(chatters[0] if chatters else 0)
            else:
                nChatters = "0"

            users_sources = [
                f"SELECT (SELECT username FROM users WHERE twitch_id=user) as user FROM history_users "
                f"WHERE date BETWEEN DATETIME('{start_time_1}') AND DATETIME('{end_time_1}')"
            ]

            if has_current_chat_table:
                users_sources.append(
                    f"SELECT (SELECT username FROM users WHERE twitch_id=user) as user FROM {table_name} "
                    f"WHERE timestamp BETWEEN DATETIME('{start_time_1}') AND DATETIME('{end_time_1}') "
                    f"GROUP BY user"
                )
            if has_previous_chat_table:
                users_sources.append(
                    f"SELECT (SELECT username FROM users WHERE twitch_id=user) as user FROM {ptable_name} "
                    f"WHERE timestamp BETWEEN DATETIME('{start_time_1}') AND DATETIME('{end_time_1}') "
                    f"GROUP BY user"
                )
        ...
        reemplazos = {
            ...
            "[streamsComparisonChart]": comparison_chart_html,
        }
