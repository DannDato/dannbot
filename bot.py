...
from Helpers.helpers_bot import read_save_chat, user_joined, send_timed_messages, handle_redeem, handle_bits
...
class TwitchBot(commands.Bot):
    ...
    async def listen_to_pubsub(self):
        animated_message("Iniciando pubsub... ","\033[38;5;204m")
        topics = [
            pubsub.channel_points(token=access_token)[int(broadcaster_id)], #Escuchar canje de bits
            pubsub.bits(token=access_token)[int(broadcaster_id)]  # Escuchar bits
        ]
    ...
    async def event_pubsub_bits(self, event):
        await handle_bits(event.amount, event.user.name)
        # Puedes personalizar lo que se dice en el canal
